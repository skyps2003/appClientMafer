...
export class PaymentFormComponentComponent implements AfterViewInit {
  ...
  getUser(){
    ...
    async ngAfterViewInit() {
      ...
      try {
        const resp: any = await this.http.post('https://api.agroindustriasmafer.com/api/checkout', { amount: total, currency: 'PEN' }).pipe(
          retry(3),
          catchError((error) => {
    ...
    private onSubmit = async (paymentData: KRPaymentResponse): Promise<boolean> => {
      try {
        const response: any = await this.http.post('https://api.agroindustriasmafer.com/api/validate', paymentData, { responseType: 'text' }).toPromise();
        if (response) {
          this.paid = true;
